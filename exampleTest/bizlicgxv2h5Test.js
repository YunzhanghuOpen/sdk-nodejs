...
class bizlicgxv2h5Test{
  ...
  constructor(dealer_info) {
      ...
      this.client = new yzhAPI.BizlicGxV2H5ServiceClient(this.dealer_info)
    }

    async GxV2H5GetStartUrl(){
      // 预启动
      const res = {
        api_name: 'GxV2H5GetStartUrl',
        api_detail: '预启动'
      };

      try {
        const data = await this.client.GxV2H5GetStartUrl({
          /**
    ...
    async GxV2H5GetAicStatus(){
      // 查询个体工商户状态
      const res = {
        api_name: 'GxV2H5GetAicStatus',
        api_detail: '查询个体工商户状态'
      };

      try {
        const data = await this.client.GxV2H5GetAicStatus({
          /**
          * @param {string} request-id：请求 ID，请求的唯一标识
          * 建议平台企业自定义 request-id，并记录在日志中，便于问题发现及排查
          * 如未自定义 request-id，将使用 SDK 中的 UUID 方法自动生成。注意：UUID 方法生成的 request-id 不能保证全局唯一，推荐自定义 request-id
    ...
    async Example(){
      const params = []
      params.push(await this.GxV2H5GetStartUrl())
      params.push(await this.GxV2H5GetAicStatus())

      params.forEach(item => {
        item.module_name = 'bizlicgxv2h5Test'
        item.module_detail = '个体工商户注册（云账户共享大额 H5）'
      });
...
module.exports = bizlicgxv2h5Test;
